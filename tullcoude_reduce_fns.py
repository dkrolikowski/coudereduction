...
# Yes this is still written in python 2... I started to convert it, but hit some snags (cosmics.py being one of them)
...
# A function I have written to fit with a sigma-rejection bspline for continuum fitting, not necessary for plain reduction
from continuumfit import getContinuum

###################################################################################################

########## The functions for running the reduction! ##########

### Function: creating the header information file ###
def Header_Info( Conf ):

    files   = glob.glob( '*.fits' ) # List of files

    outfile = open( Conf.InfoFile, 'wb' ) # Open the output file
    ...
    for f in range( len( files ) ):
        ...
        itype = head["imagetyp"] # Get the image type (object, comp, zero, flat)
        if len(files[f].split('.')) > 2: itype = "unknown"

        ra = ''
        dec = ''
        if "RA"  in head.keys(): ra  = head["RA"] # Header RA/Dec coordinates
        if "DEC" in head.keys(): dec = head["DEC"]

        air = '100'
        if "airmass" in head.keys(): air = str(head["airmass"]) # Airmass

        exptime = str(head["exptime"]) # Exposure time in seconds

        if "GAIN3" in head.keys(): # Gain and read noise
            gain = str(head["gain3"])
...
### Function: making the combined bias frame ###
def Build_Bias( files, readnoise ):

    testdata = fits.open( files[0] )[0].data
    biascube = np.zeros( ( len( files ), testdata.shape[0], testdata.shape[1] ) ) # Setting the output array

    for f in range( len( files ) ):
        biascube[f] = fits.open( files[f] )[0].data # Read in bias images

    SuperBias = {}

    SuperBias['vals'] = np.nanmedian( biascube, axis = 0 ) # Get the median of the bias
    SuperBias['errs'] = np.sqrt( SuperBias['vals'] + readnoise ** 2.0 ) # Error
...
def Build_Flat_Field( files, readnoise, SuperBias ):
    ...
    testdata = fits.open( files[0] )[0].data
    flatcube = np.zeros( ( len( files ), testdata.shape[0], testdata.shape[1] ) ) # Setting the output array

    for f in range( len( files ) ):
        flatcube[f] = fits.open( files[f] )[0].data # Read in flat images

    FlatField = {}

    flatmedian = np.nanmedian( flatcube, axis = 0 ) # Get the median of the bias
    ...
    FlatField['vals'] = flatvalues - flatvalues.min() # Subtract minimum from the flat
    FlatField['errs'] = FlatField['errs'] / FlatField['vals'].max() # Scale by the maximum of the flat field
    FlatField['vals'] = FlatField['vals'] / FlatField['vals'].max() # Scale by the maximum of the flat field, now ranges from 0 to 1
...
def Make_BPM( SuperBias, FlatField, Conf ):
    ...
    cutbias = np.percentile( SuperBias, Conf.BPMlimit ) # Find where bias pixels are above a percentile cut off
    BPM     = np.where( ( SuperBias > cutbias ) | ( FlatField <= 0.0001 ) ) # Also if the flat field is super duper small

    return BPM

### Function: Call sub-functions and return combined bias, combined flat, BPM ###
def Basic_Cals( BiasInds, FlatInds, FileInfo, Conf ):

    # If we need to generate calibration files
    if Conf.doCals == True:
    ...
    # If we're reading in already generated cal files
    elif Conf.doCals == False:
...
########## Section: Generate data cubes -- 2D spectral cubes for thar and object exposures, with calibrations done ##########

### Function: Make the data cubes! With calibration, cosmic subtraction, error propagation ###
def Make_Cube( Files, ReadNoise, Gain, DarkVal, Conf, Bias = None, Flat = None, BPM = None, cossub = False ):
    ...
    for f in range( len( Files ) ): # Loop through all files
        frame = fits.open( Files[f] )[0].data # Read in the data

        if f == 0:
            Cube = np.zeros( ( len( Files ), frame.shape[0], frame.shape[1] ) ) # Set up arrays for the output value and S/N cubes
        ...
        # Perform cosmic subtraction, if directed to
        if cossub:
        ...
        # Some holder variables for uncertainty calculation
        ...
        if BPM is not None: # If there's a bad pixel
            Cube[f,BPM[0],BPM[1]] = np.nanmedian( Cube[f] )  # Set a bad pixel to the median (...?)
            SNR[f,BPM[0],BPM[1]]  = 1e-4 # Well I'm setting the S/N to effectively 0 so...

        wherenans = np.where( np.isnan( Cube[f] ) ) # Get rid of any nans
...
### Function: Call Make_Cube to return ThAr and object cubes ###
...
def Start_Trace( flatslice, percent ):
    ...
    orderzeros = [] # Array to hold where orders start
    last       = 0
    ...
    for i in range( 6, flatslice.shape[0] ):
        if fgrad[i] > cutvalue or last == 0:
            if 100 > i - last > 20 or last == 0:
                orderzeros.append( i + 11 ) # 11 is roughly half of an order's pixel height
                last = i

    orderzeros = np.array( orderzeros )

    # Go through and recenter the peaks found
    for i in range( len( orderzeros ) ):
        o      = orderzeros[i]
        cutoff = flatslice[o] * ( 0.7 ) # Look for where the flat slice is above 70% of the peak height for recentering
...
#    plt.show()
    ...
    # By hand remove extra orders that are present at the midpoint (here is another example of the hard coding, this just gets rid of extra orders in the middle)
    midzeros = midzeros[2:]
    midvals  = midvals[2:]
    ...
    for i in range( 5, 50 ): # fit these slopes over some middle range of orders
        dy = float( startzeros[i] - midzeros[i] )
        slopes.append( dy / dx )

    slopefit = np.polyfit( range( 5, 50 ), slopes, 2 )

    # Get the final starting locations of the orders
...
def Full_Trace( brightcube, orderzeros, orderstart ):
    ...
    for f in range( brightcube.shape[0] ): # Use all of the object frames above a certain brightness (percentile chosen)
...
def Get_Trace( Flat, Cube, Conf ):
    ...
    if Conf.doTrace == True:
        ...
        # Determine the brightest object frames for trace finding (with median counts above some perctange of median counts of all object frames)
        ...
        trace     = Full_Trace( abovemed, orderzeros, orderstart ) # Get full trace (N_obs x N_ord x N_pix array)
        ...
        # Output the median and fitted trace
    ...
    # If trace has already been calculated
    elif Conf.doTrace == False:
...
def OrderModel( X, p, return_full = False ):
    ...
    # Full model: 2D gaussian
    model  = peaks * np.exp( - ( y - means ) ** 2.0 / ( 2.0 * sigmas ** 2.0 ) ) + p[10]

    if return_full == False:
        return model    # If we just want the model
    else:
        return model, means, peaks, sigmas # If we need all of the model constituents
...
def Extractor( Cube, SNR, Trace, Conf, quick = True, arc = False, nosub = True ):
    ...
    for frm in range( Cube.shape[0] ):
        ...
        thisfrm = Cube[frm,:,:] # Get the cube values and S/N for the frame
        thissnr = SNR[frm,:,:]
        ...
        for ord in range( Trace.shape[0] ):
            ...
            # Set up rectangle around order that will be fit -- 16 pixels wide
            ...
            # If we're doing the full extraction!
            if (quick == False) & (arc == False):

                # Clean obvious high outliers, I commented this out, but might wanna reinstate it (not necessarily redoing what cosmics does)
#                toohigh         = np.where( tblock > 15.0 * np.median( tblock ) ) # Where frame counts are too high (arbitrary)
#                tblock[toohigh] = np.median( tblock )
#                tsnr[toohigh]   = 0.000001 # Set to low S/N value

                # Get just the error block ( signal / ( signal / noise ) ), absolute because it has sometimes been negative, probably not good
                tnoise = np.absolute( tblock / ( tsnr ) )

                # Clean zero values (often due to chip artifacts that aren't caught)
                if np.median( tblock ) <= 0.0:
                    val2use = 0.0 # Will use median to replace values too low, but if median is somehow below 0, set them to 0
                else:
                    val2use = np.median( tblock )
                ...
                # Change the values in the noise block, honestly... what is going on here lol
                if val2use == 0.0:
                    tnoise[toolow] = np.median( np.abs( tblock[tblock>0] ) )
                else:
                ...
                if ordres['status'] == -16: # Fail state if the fit doesn't work (although I dunno if this has ever happened)
            ...
            # Now go pixel by pixel along order! To collapse 2D to 1D
            for pix in range( Trace.shape[1] ):

                slice          = tblock[pix,:] # Get the pixel slice values and S/N
                slice_snr      = tsnr[pix,:]
                qwe            = np.where( slice_snr < 0.0 )[0] # If SNR anywhere is negative for some reason... This should hopefully never be used?
                slice_snr[qwe] = 1e-5                           # Set the SNR to something very small!
                thisx          = np.arange(len(slice)) # The index locations along the slice
                snoise         = np.absolute(slice/slice_snr) # The errors themselves
                snoise[snoise==0.0] = 0.00001 # Don't have error of 0!
                ...
                if quick == False:
                    ...
                    # Do 1D Gauss fit of the pixel slice
                    slicepars, sliceres  = mpyfit.fit( Least, p0, ( thisx, slice, snoise, GaussModel ), parinfo = parinfo, maxiter = 10 )

                    savesigma[pix] = slicepars[2] # Output the values of the fit
                    savespot[pix]  = slicepars[1]
                    savepeak[pix]  = slicepars[0]
                    savebg[pix]    = slicepars[3]
                    bg             = slicepars[3]

                    themod = GaussModel( thisx, slicepars ) # Get best fit 1D gaussian model and residuals
                    ...
                    if len(bads) > 3:
                        slice_snr = slice_snr * 0.0 + 1e-5 # If there are more than 3 bad points, downweight pixel slice. Maybe look into this?

                # If we're doing quick extraction, simple background subtraction
                if quick   == True:
                    bg = slice * 0.0 + np.min( np.absolute( slice ) ) # If quick subtraction, set background to minimum of slice
                if nosub   == True:
                    bg = slice * 0.0 # If no subtraction set, set background to 0
                ...
                if arc == True:
                    flux[frm,ord,pix] = np.sum( cslice )
...
########## Section: wavelength calibration, thar solution and apply to objects ##########
...
def Plot_Order_Wavsol( wavsol, wavkept, spec, THAR, path, frame, order ):
    ...
    plt.clf()
    plt.plot( wavsol, spec, 'k-', lw = 1 ) # Plot the data
    plt.plot( THAR['wav'], THAR['logspec'], 'r-', lw = 1 ) # Plot the reference ThAr spectrum
    plt.xlim( wavsol[0] - 5.0, wavsol[-1] + 5.0 )
    for peak in wavkept:
        plt.axvline( x = peak, color = 'b', ls = ':', lw = 1 ) # Plot the peaks that we find and use in the solution!!
    plt.savefig( path + '/fullspec_ord_' + str(order) + '.pdf' ); plt.clf()

    # Now do the spectrum window plots, for zoom in!

    numfigs = np.ceil( np.ceil( np.ptp( wavsol ) / 10.0 ) / 6.0 ).astype(int) # Set the number of subplots for a given wavelength range
    start   = np.min( wavsol )
    if numfigs > 4: # I guess I don't allow that many plots? I dunno lol
        return None
    else:
        for i in range(numfigs):
            ...
            while start <= np.max( wavsol ) and j < 7:
                ...
                window_min = start # Get the wavelength window (with some overlap)
                ...
                plt.plot( wavsol[data_loc], spec[data_loc] + 0.4 * y_offset, 'k-', lw = 1 ) # Data
                plt.plot( THAR['wav'][ref_loc], THAR['logspec'][ref_loc], 'r-', lw = 1 ) # Reference ThAr spectrum
                for peak in wavkept:
                    plt.axvline( x = peak, color = 'b', ls = ':', lw = 1 ) # Peaks
...
def Plot_WavSol_Resids( resids, lines, cutoff, savename, tokeep = None, toreject = None ):
    ...
    # Median absolute deviation in wavelength and velocity (there's a scipy function to do this)
    ...
    wavax.axhline( y = 0.0, color = 'k', ls = ':' ) # Lines are residuals of 0
    velax.axhline( y = 0.0, color = 'k', ls = ':' )
    if toreject is None:
        wavax.plot( lines, resids['wav'], 'ko', mfc = 'none' ) # Plot the peaks that are kept
        velax.plot( lines, resids['vel'], 'ko', mfc = 'none' )
        fig.suptitle( 'Lines Used: ' + str(lines.size) + ', Cutoff: ' + str(cutoff * 0.67449) + ' $\sigma$' )
    elif tokeep is None:
        wavax.plot( lines, resids['wav'], 'kx', mfc = 'none' ) # Plot the peaks that are rejected
...
def Find_Peaks( wav, spec, specsig, peaksnr = 5, minsep = 0.5, pwidth = 10 ):
    ...
    for peak in peaks:
        ...
        xi   = wav[peak - pwidth:peak + pwidth] # Estimate of the peak wavelength from previous wavelength solution
        yi   = spec[peak - pwidth:peak + pwidth]
        sigi = specsig[peak - pwidth:peak + pwidth]
        inds = np.arange( len(xi), dtype = float )

        # Fit the gaussian in index space
...
### Function: Smoothing arc spectra for purposes of offset from the initial guess wavelength solution ###
...
### Function: Getting the shift from the preliminary wavelength solution ###
...
def Get_WavSol( Cube, CubeSig, Conf, plots = True, Frames = 'All', Orders = 'All' ):
    ...
    if not Conf.doArcWav:
        ...
        # orderdif = 0 # I think this is right for Bill's wavelength solution. (no offset)
        ...
        for frame in frameloop:
            ...
            for order in orderloop:
                ...
                arcspec    = smoothcube[frame,order,:] # Wait why am I using the smoothed spectrum...? lol
                arcsigma   = smoothsig[frame,order,:]
                prelimsol  = roughsol[order+orderdif,:] # Initial wavelength guess
        ...
        for f in range( badorders.shape[0] ):
            print( 'Bad orders for frame', str(f), np.where( badorders[f] )[0] )
...
def Interpolate_Obj_WavSol( wavsol, info, arcinds, objinds, Conf ):
    ...
    if not Conf.doObjWav: # If done, just read in previous object wavelength
        ...
        for i in range( len( objtime ) ):
            ...
            else: # Do the interpolation between two closest arcs
...
def doContinuumFit( spec, sigspec, Conf, obj_filename ):
    ...
    for f in range( spec.shape[0] ): # Loop through the frames
        for o in range( spec.shape[1] ): # Loop through the orders
            ...
            # Roughly correct parameters for the spline fitting, but should be object specific so really this is just a rough attempt at the continuum
